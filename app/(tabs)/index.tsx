...
const EXPENSES_INDEX_ROUTE = '/expenses' as Href;
...
export default function HomeScreen() {
  ...
  useEffect(() => {
    let cancelled = false;
    async function loadCurrentTripExpenses() {
      if (!selectedTripId) {
      ...
      try {
        const expenses = await listExpenses(() => getTokenRef.current(), selectedTripId);
        if (!cancelled) setExpensesState({ kind: 'ready', expenses });
      } catch (e) {
        if (!cancelled) {
          setExpensesState({
            kind: 'error',
    ...
    void loadCurrentTripExpenses();
    return () => {
      cancelled = true;
    };
  }, [selectedTripId]);

  const handleTripPickerResult = useCallback((result: TripPickerResult) => {
    if (result.type !== 'select_trip') return;
    setSelectedTripId(result.tripId);
    void AsyncStorage.setItem(CURRENT_TRIP_STORAGE_KEY, result.tripId);
  ...
  const heroMetaLeft = tripsLoading
    ...
    : selectedTrip
      ...
      : trips.length === 0
        ? 'NO TRIPS'
        : 'RECENT';
  const heroMetaCenter = selectedTrip?.name ?? 'evenly';
  const showHeroAmountSkeleton = tripsLoading || expensesState.kind === 'loading';
  const showViewAll = sortedExpenses.length >= 6;
  ...
  return (
    <AppScreen>
      ...
      <SectionHeader
        ...
        trailing={
          showViewAll ? <ViewAllLink onPress={() => router.push(EXPENSES_INDEX_ROUTE)} /> : null
        }
      />
