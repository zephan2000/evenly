...
- Soft deletes: `deleted_at timestamptz`. Always filter `deleted_at IS NULL` in queries. **A soft-delete UPDATE must emit zero `RETURNING`.** SELECT policies carry `deleted_at IS NULL`; Postgres re-applies the SELECT policy to any row a mutation returns, so the just-soft-deleted row fails it with `new row violates row-level security policy`. This is triggered by PostgREST `.select()` **and** by `{ count: 'exact' }` (PostgREST counts via `WITH s AS (UPDATE … RETURNING *) SELECT count(*) FROM s` — the CTE's `RETURNING *` is still policy-checked). Correct pattern: a pre-check `SELECT` (same SELECT policy decides not-found/forbidden) **then** a bare `UPDATE` with no `.select()` and no count (PostgREST `return=minimal` → plain `UPDATE`, no `RETURNING`). UPDATE policies' `WITH CHECK` must likewise exclude `deleted_at IS NULL`.
